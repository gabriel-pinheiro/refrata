import { settings, type OutputKind } from "@refrata/core";

import type { SerialLink, SerialOptions } from "./serial-link.ts";

/**
 * How each widget family takes a DMX Frame. An Open DMX widget is a bare
 * serial line: the host raises the break, releases it and streams the start
 * code plus 512 slots at 250 kbaud with two stop bits. A DMX USB Pro takes a
 * framed packet (label 6, little-endian length, payload, end byte) over an
 * ordinary serial link and times the wire itself.
 */
...
export const openDmxDriver: OutputDriver = {
...
export const usbProDriver: OutputDriver = {
  options: { baudRate: 57_600, dataBits: 8, stopBits: 1, parity: "none" },
  send: (link, frame) => link.write(usbProPacket(frame)),
};

export const DRIVERS: Record<OutputKind, OutputDriver> = {
  "enttec-open-dmx": openDmxDriver,
  "enttec-usb-pro": usbProDriver,
};
