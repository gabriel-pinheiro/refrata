...
/**
 * Firing a trigger Address. A Macro's run performs its actions; any other
 * trigger is an event the runtime announces to every session subscribed to
 * the document. All of it is show input: never undone, and events are never
...
export const addressTrigger = defineCommand({
  ...
  apply({ document, payload }) {
    const fired = fireAddress(document, payload.address);
